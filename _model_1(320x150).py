...
img_width = 320  # 200  # 320  # 320 # Resized Video width to be handled
img_height = 150  # 66  # 150  # 150 # Resized Video height to be handled
...
with tf.name_scope("CNN_3") as scope:
    ...
    '''
    L3 = tf.reshape(L3, [-1, 5 * 10 * 128])

with tf.name_scope("FCL_1") as scope:
    shape = [5 * 10 * 128, 625]  # ??? 가로필터 x 세로필터 x 색상 x 필터수
    W4 = weight(name="W4", shape=shape, w_type = 0)
    shape = [625]  # ??? 가로필터 x 세로필터 x 색상 x 필터수
...
with tf.name_scope("FCL_2") as scope:
    shape = [625, nb_classes]  # ??? 가로필터 x 세로필터 x 색상 x 필터수
    W5 = weight(name="W5", shape=shape, w_type = 0)
    shape = [nb_classes]  # ??? 가로필터 x 세로필터 x 색상 x 필터수
